...
class TreeNode:
    def __init__(self, level, cluster_id, texts, parent=None):
        ...
        self.texts = texts  # The original texts in this cluster
        self.summary = None  # Summary of this cluster
        self.children = []  # Child nodes for further abstractions
        self.parent = parent
...
class RAPTORPipeline:
    def __init__(self, embedding_model, llm, chromadb_handler, depth=3, n_clusters=5):
        """
        Initialize the RAPTOR pipeline with the embedding model, language model,
        ChromaDB handler, and configuration for hierarchical depth and clusters.
        """
    ...
    def evaluate_clusters(self, embeddings, max_clusters=10):
        ...
        if max_possible_clusters < 2:
            return 1  # Return 1 cluster if we don't have enough samples
        ...
        if not silhouette_scores:
            return 1  # Return 1 cluster if we couldn't calculate scores
    ...
    def reduce_dimensions(self, embeddings):
        """Reduce dimensions of embeddings using UMAP with adaptive n_components."""
        n_samples, n_features = embeddings.shape

        # Calculate appropriate number of components
        # Use min of: number of samples-1, original dimensionality, and 50
        n_components = min(n_samples - 1, n_features, 50)

        if n_components < 2:
            return embeddings  # Return original embeddings if we can't reduce

        try:
            reducer = umap.UMAP(
                n_components=n_components,
                n_neighbors=min(15, n_samples - 1),  # Adjust n_neighbors if needed
                min_dist=0.1,
                metric='euclidean'
            )
    ...
    def recursive_abstraction(self, texts, level=0, parent_node=None):
        """Recursively clusters and summarizes documents into hierarchical abstractions."""
        if level >= self.depth or len(texts) <= 1:
            return TreeNode(level=level, cluster_id=0, texts=texts, parent=parent_node)

        # Embed texts
        embeddings = self.embedding_model.embed(texts).cpu().numpy()
        ...
        n_samples = len(texts)
        max_possible_clusters = min(10, n_samples - 1)
        best_n_clusters = self.evaluate_clusters(reduced_embeddings, max_clusters=max_possible_clusters)

        if best_n_clusters <= 1:
            # If we can't create multiple clusters, return a leaf node
            return TreeNode(level=level, cluster_id=0, texts=texts, parent=parent_node)
        ...
        current_node = TreeNode(level=level, cluster_id=0, texts=texts, parent=parent_node)
        ...
        for cluster_id, cluster_texts in clustered_texts.items():
            ...
            # Create and link child node
            child_node = self.recursive_abstraction(cluster_texts, level=level + 1, parent_node=current_node)
            child_node.set_summary(summary)
            current_node.add_child(child_node)

        return current_node

    def process_and_add_to_db(self, texts, doc_ids):
        """Process texts and add the resulting hierarchy to the database."""
        if not texts:
        ...
        try:
            ...
            root_node = self.recursive_abstraction(texts)

            # Add to database
            self.add_hierarchy_to_db(root_node, doc_ids)
        except Exception as e:
            logger.error(f"Error in processing and adding to database: {e}")
            raise

    def add_hierarchy_to_db(self, root_node, doc_ids):
        """Add the recursive abstraction hierarchy to the vector DB."""

        def traverse_and_add(node, level):
            if node.is_leaf():
                return

            # Create a unique cluster identifier
            cluster_id = len(doc_ids) if not hasattr(node, 'cluster_id') else node.cluster_id

            # Only add if there's a summary
            if node.summary:
                try:
                    summary_embedding = self.embedding_model.embed([node.summary]).cpu().numpy()
                    metadata = {
                    ...
                    self.chromadb_handler.add_embeddings(
                        ...
                        metadatas=[metadata],
                        ids=[f"cluster_{cluster_id}_level_{level}"]
                    )
                except Exception as e:
                    logger.error(f"Error adding node to database: {e}")

            # Process children
            for child in node.children:
                traverse_and_add(child, level + 1)

        traverse_and_add(root_node, level=0)

    def retrieve_documents(self, query, max_depth=None):
        ...
        try:
            ...
            for depth in range(max_depth + 1):
                results = self.chromadb_handler.query(
...
if __name__ == "__main__":
    ...
    raptor_pipeline.process_and_add_to_db(chunked_texts, document_ids)

    # Process texts and add to database
    try:
        raptor_pipeline.process_and_add_to_db(chunked_texts, document_ids)
    except Exception as e:
